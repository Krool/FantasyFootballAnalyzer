...
export async function loadLeague(leagueId: string): Promise<League> {
  ...
  // Calculate started points for each player across all matchups
  const playerStartedPoints = new Map<string, { points: number; games: number }>();
    ...
    weekMatchups.forEach(matchup => {
      if (matchup.starters && matchup.starters_points) {
        matchup.starters.forEach((playerId, index) => {
          ...
          const existing = playerStartedPoints.get(playerId) || { points: 0, games: 0 };
          playerStartedPoints.set(playerId, {
  ...
  allTransactions
    ...
    .forEach(tx => {
      ...
      // Calculate total points generated by added players (in started games)
      let totalPointsGenerated = 0;
      let gamesStarted = 0;
      adds.forEach(player => {
        const startedData = playerStartedPoints.get(player.platformId);
        if (startedData) {
          totalPointsGenerated += startedData.points;
          gamesStarted += startedData.games;
        }
      });
